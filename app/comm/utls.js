...
exports.encodedEntry = function (bucket, key) {
  return exports.urlsafeBase64Encode(bucket + (
      key ? ':' + key : ''
  ));
};
...
// @param reqContentType 请求类型，例如 application/json 或者
// application/x-www-form-urlencoded @param reqBody        请求Body，仅当请求的
// ContentType 为 application/json 或者  application/x-www-form-urlencoded
// 时才需要传入该参数
exports.generateAccessTokenV2 = function (
    mac, requestURI, reqMethod, reqContentType, reqBody) {
  var u     = url.parse(requestURI);
  var path  = u.path;
  var query = u.query;
  var host  = u.host;
  ...
  // add content type
  if (reqContentType && (
          reqContentType == 'application/json' || reqContentType
